...
public class IOProcessorPlugin implements IResolvable<IOProcessorGlobal,IOProcessorLocal>, FProPluginList {
	...
	public static final String			PLUGIN_DESCRIPTION = "This plugin supports a set of I/O predicates and operators";
	...
	public static final char[]			PREDICATE_SPLIT = "split1(String, Divizor, List).".toCharArray(); 
	public static final char[]			PREDICATE_LIST = "inList1(String, List).".toCharArray(); 
	...
	private long				splitId, listId, writeId, writeLnId, charArrayId;
	...
	public IOProcessorGlobal onLoad(final LoggerFacade debug, final SubstitutableProperties parameters, final IFProEntitiesRepo repo) throws SyntaxException {
		try(final LoggerFacade 				actualLog = debug.transaction("IOProcesorPlugin:onLoad")) {
			...
			try{
				...
				pap.parseEntities(PREDICATE_WRITE, 0, (entity, vars) -> {
					writeId = entity.getEntityId();
					repo.pluginsRepo().registerResolver(entity, vars, IOProcessorPlugin.this, global);
					return true;
				});
				actualLog.message(Severity.info,"Predicate "+new String(PREDICATE_WRITE)+" was registeded successfully");

				pap.parseEntities(PREDICATE_WRITELN, 0, (entity, vars) -> {
					writeLnId = entity.getEntityId();
					repo.pluginsRepo().registerResolver(entity, vars, IOProcessorPlugin.this, global);
					return true;
				});
				actualLog.message(Severity.info,"Predicate "+new String(PREDICATE_WRITELN)+" was registeded successfully");
				...
				pap.parseEntities(OPERATOR_CHARARRAY, 0, (final IFProEntity entity, final List<IFProVariable> vars) -> {
					...
					repo.pluginsRepo().registerResolver(op, vars, IOProcessorPlugin.this, global);
	...
	public ResolveRC firstResolve(final IOProcessorGlobal global, final IOProcessorLocal local, final IFProEntity entity) throws SyntaxException {
		if (entity.getEntityType() == EntityType.predicate) {
			...
			else if (entity.getEntityId() == writeId && ((IFProPredicate)entity).getArity() == 1) {
				final IFProEntity[]		parms = ((IFProPredicate)entity).getParameters();
				final StringBuilder 	sb = new StringBuilder();
				final CharacterTarget	ct = new StringBuilderCharTarget(sb);
				
				try{global.pap.putEntity(parms[0], ct);
			...
			else if (entity.getEntityId() == writeLnId && ((IFProPredicate)entity).getArity() == 1) {
				final IFProEntity[]		parms = ((IFProPredicate)entity).getParameters();
				final StringBuilder 	sb = new StringBuilder();
				final CharacterTarget	ct = new StringBuilderCharTarget(sb);
	...
	private static ResolveRC firstResolveSplit(final IOProcessorGlobal global, final IOProcessorLocal local, final IFProEntity mark, final IFProEntity first, final IFProEntity second, final IFProEntity third) {
	...
	private static ResolveRC firstResolveList(final IOProcessorGlobal global, final IOProcessorLocal local, final IFProEntity mark, final IFProEntity first, final IFProEntity second) {
		final FProUtil.Change[]		change = new FProUtil.Change[1];
	...
	private ResolveRC nextResolveList(final IOProcessorGlobal global, final IOProcessorLocal local, final IFProEntity mark) {
